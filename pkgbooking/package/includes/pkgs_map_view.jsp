...
<%@page import="com.eos.accounts.user.UserPreferencesThreadLocals"%>
...
<%
	...
	String query = searchQuery.getQueryStr();
...
.ctyDescV .plcCtr h3 {font-size:13px; font-weight:bold; color:#333; padding:0 0 8px;}
.ctyDescV .plcCtr li {list-style-type:disc; margin-left:18px;}
.ctyDescV .plcCtr li span {font-size:11px !important;}
...
.mapFltrCtr .fltrCtr .grBtn5.selected {background:#FCF9D7;}
.mapFltrCtr .fltrCtr a.selected {font-weight:bold;}
.mapFltrCtr .nvDwnIm, .htlFCtr .nvDwnIm {font-size:11px;}
.mapFltrCtr .plLstNv li, .htlFCtr .plLstNv li {font-size:11px;}
...
<div id="pkgsMapVw" style="position:absolute; top:0; bottom:0; left:0; right:0;">
	<div class="fltrCtr mapFltrCtr u_block"></div>
...
$jQ(document).ready(function() {
	var pkgV = new PkgsView({ctr:'#pkgsMapVw', query:'<%=query%>', itnView:<%=searchQuery.getViewType() == SellableContentSearchViewType.ITINERARY_MAP%>, viewT:'<%=searchQuery.getViewType().name().toLowerCase()%>', currency:'<%=CurrencyType.getShortCurrencyCode(UserPreferencesThreadLocals.getCurrency())%>'});
	pkgV.fetchResults('<%=query%>', <%=filtersStr != null ? "'" + filtersStr + "'": "null"%>);
	pkgV.setSearchForm(document.holidaySearchForm);
});
</script>
