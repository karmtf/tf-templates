...
<%
	...
	Map<WegoHotel, Integer> hotels = (Map<WegoHotel, Integer>)request.getAttribute("products");
...
<div id="mainContent" class="mainContent u_floatL" style="width:740px;">
	...
	<div class="partnerBlk">
		<form class="form-horizontal" name="packageForm" class="packageForm" action="<%=PagesRequestURLUtil.getActionURL(request, SubNavigation.PARTNER, PartnerAction.LOAD_HOTELS)%>" method="post">
		...
		<div class="widget">
			...
			<div class="form-actions align-right">
				...
				<div class="u_floatL" style="margin-left:10px"><button type="button" class="btn btn-primary" onclick="submitHotel()">Map Hotel</button></div>
				<% if(loggedInUser.getRoleType() == RoleType.ADMIN || loggedInUser.getUserId() == 1645133) { %>
				<div class="u_floatL" style="margin-left:10px"><button type="button" class="btn btn-primary" onclick="downloadHotels()">Download Hotels</button></div>
					...
					<tbody>
						<%
							...
							if(hotels != null && !hotels.isEmpty()) { 
								for (WegoHotel hotel : hotels.keySet()) {
									int mappedId = hotels.get(hotel);
									if(hotel != null) { 
						%>
						<% if(mappedId > 0) { %>
						<td></td>
						<% } else { %>
						<td><input type="radio" name="maphotelid" value="<%=hotel.getHotelId()%>" /></td>
						<% } %>
						<td><%=hotel.getName()%></td>
						<% if(mappedId > 0 &&  MarketPlaceHotel.getHotelById(mappedId) != null) { %>
						<td><a target="_blank" href="<%=HotelDataBean.getHotelDetailsURL(request, MarketPlaceHotel.getHotelById(mappedId))%>">Details</a></td>
						<% } else { %>
						<td>No</td>
						<% } %>
						<td>
							<% if(mappedId > 0) { %>
							<a href="/partner/manage-hotel?hotelid=<%=mappedId%>" class="tip" title="Edit Property Details">Edit</a>
							<% } %>
						</td>
						<td>
							<% if(mappedId > 0) { %>
							<% } else { %>
							<a href="/partner/add-new-hotel?hotelid=<%=hotel.getId()%>&destid=<%=hotel.getCityId()%>" class="tip" title="Edit Property Details">Add New Hotel</a>
							<% } %>
						</td>
						</tr>
						<% } } } %>
