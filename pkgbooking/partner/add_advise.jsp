...
<%
	...
	long country = -1L;
	if(lhsState != null && lhsState.getUserInputForInputType(UserInputType.CITY_PARENT, true).getValue() != null) { 
		country = (Long)lhsState.getUserInputForInputType(UserInputType.CITY_PARENT, true).getValue();
	}
...
<div id="mainContent" class="mainContent u_floatL" style="width:740px;">
	...
	<h5 class="widget-name">Add New Travel Recommendation</h5>
	<form class="form-horizontal" name="packageForm" class="packageForm" action="/partner/add-advise" method="post">
		<% if(relid > 0) { %>
		...
		<input type="hidden" name="sourceCity" value="<%=sourceCity%>" />
			...
			<div class="well">
				...
				<div class="statement-group" style="margin-bottom:10px">	
					<div class="control-group">
						...
						<div class="controls"><%=kr.getRelationshipName()%></div>
					</div>
					<% if(!product.equals("holiday")) { %>
					<div class="control-group">
						<label class="control-label">For City</label>
						<div class="controls"><input type="text" value="<%=(cityId > 0) ? LocationData.getCityNameFromId(cityId) : ""%>" placeholder="Enter a city name" name="dCityEx" id="dCityEx" class="ui-autocomplete-input span12" autocomplete="off" /></div>
					</div>
					<% } else { %>
					<div class="control-group">
						<label class="control-label">Ex City</label>
						<div class="controls"><input type="text"  value="<%=sourceCity > 0 ? LocationData.getCityNameFromId(sourceCity):""%>" name="sCity" id="sCityEx" class="ui-autocomplete-input span12" autocomplete="off" /></div>
					</div>
					<div class="control-group">
						<label class="control-label">OR</label>
					</div>
					<div class="control-group">
						<label class="control-label">Into Country</label>
						<div class="controls"><input type="text"  value="<%=country > 0 ? DestinationContentManager.getDestinationNameFromId(country):""%>" name="countryEx" id="countryEx" class="ui-autocomplete-input span12" autocomplete="off" /></div>
					</div>
					<% } %>
				</div>
				...
				<div class="statement-group">
					<%
						...
						String name = "";
						int max = 25;
						if(user.getRoleType() == RoleType.EXPERT || user.getRoleType() == RoleType.TOUR_OPERATOR || user.getRoleType() == RoleType.TRAVEL_AGENT) {
							max = 10;
						}
						for(int i = 1; i <= max; i++) {
							if(places != null && !places.isEmpty() && iter.hasNext()) {
								...
								MarketPlaceHotel hotel = MarketPlaceHotel.getHotelById(hotelId);
								name = (hotel != null) ? hotel.getName(): "Missing Hotel - " + hotelId;
							} else {
								placeId = -1L;
								name = "";
								hotelId = -1;
					...
					<div class="control-group">
						...
						<div class="controls"><input type="text" value="<%=name%>" name="place<%=i%>" class="place" id="place<%=i%>" class="ui-autocomplete-input span12" autocomplete="off" style="width:100%" /><input type="hidden" value="<%=placeId >  0 ? placeId : (hotelId > 0 ? hotelId : "-1")%>" name="place<%=i%>dest" id="place<%=i%>dest" class="ui-autocomplete-input span12" autocomplete="off" /></div>
					</div>
					<% } %>
				</div>
...
$jQ("#countryEx").autocomplete({
   ...
   select: function(event, ui) {
	  document.packageForm.country.value = ui.item.data.id;
   }
});
