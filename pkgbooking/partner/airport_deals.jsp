...
<%
	...
	CurrencyType currencyType = selectedDeal != null ? CurrencyType.getCurrencyByNameOrCode(selectedDeal.getCurrency()) : null;
	Double amount = selectedDeal != null ? selectedDeal.getIndicativePriceAmount() : null;
%>
<html>
<head>
<title>Add Sellable Services</title>
...
<div id="mainContent" class="mainContent u_floatL" style="width:740px;">
	...
	<form name="dealsForm" action="/partner/load-airline-deals" method="get">
	<input type="hidden" name="roomid" value="0" />
	</form>
	<% if(retDealsList != null && !retDealsList.isEmpty()) { %>
	<h5 class="widget-name">Current Services Being Sold</h5>
	<table class="table">
		<tr>
			<th>Type</th>
			<th>Description</th>
		...
		<tr class="<%=(isOdd)?"odd":"even"%>">
			...
			<td style="width:300px;;font-size:11px"><%=retDeal.getDealDesc()%></td>
			<td style="width:150px"><%=retDeal.getCurrency()%> <%=retDeal.getIndicativePriceAmount()%> (<%=(retDeal.getIndicativePriceKey() != null)?retDeal.getIndicativePriceKey().getDesc()[0]:""%>)</td>
			<td style="width:50px"><a href="/partner/load-airport-deals?selectedSideNav=packages&dealId=<%=retDeal.getId()%>">Edit</a></td>
			<% if(retDeal.isDisabled()) { %>
			<td style="width:50px"><a href="/partner/load-airport-deals?selectedSideNav=packages&dealId=<%=retDeal.getId()%>&enable=true">Enable Sale</a></td>
			<% } else { %>
			<td style="width:50px"><a href="/partner/load-airport-deals?selectedSideNav=packages&dealId=<%=retDeal.getId()%>&enable=false">Disable Sale</a></td>
			<% } %>
		</tr>
		<% } %>
	</table>
	<% } %>
	<div class="spacer"></div>
	<h5 class="widget-name">Add Airport Services</h5>
	<form class="form-horizontal" name="packageForm" action="/partner/add-airport-deals" method="post">
		<input type="hidden" name="dealid" value="<%=(selectedDeal != null)?selectedDeal.getId():"-1"%>" />
		<input type="hidden" name="airportCode" value="SIN" />
		<fieldset>
		<!-- General form elements -->
		<div class="widget row-fluid">
			<div class="navbar">
				<div class="navbar-inner">
					<h6>Add New Airport Service</h6>
				</div>
			</div>
			<div class="well">
				...
				<div class="control-group">
					...
					<div class="controls">
						...
						<% for (SellableUnitType  unitType: SellableUnitType.AIRPORT_ANCILLARY_OPTIONS) { %>
				...
				<div class="control-group">
					<label class="control-label">Max Capacity (if applicable)</label>
					<div class="controls">
						<input type="text" name="qty" class="span12" id="qty" value="<%=(selectedDeal != null)?selectedDeal.getApplicableQty():""%>" style="width:50px"/>&nbsp;persons
					</div>
				</div>
				<div class="control-group">
						...
						<div class="row-fluid">
							...
							<div class="span2">
								<select name="pricingType" id="pricingType">
									<option value="<%=AllowedPricingPredefined.PRICE_FULL_AMOUNT.getDesc()[0]%>"><%=AllowedPricingPredefined.PRICE_FULL_AMOUNT.getDesc()[0]%></option>
									<option value="<%=AllowedPricingPredefined.PRICE_PER_PAX_ADULT.getDesc()[0]%>"><%=AllowedPricingPredefined.PRICE_PER_PAX_ADULT.getDesc()[0]%></option>
								</select>
							</div>
						</div>
					</div>
