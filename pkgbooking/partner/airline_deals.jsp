...
<html>
<head>
<title>Manage Ancillaries</title>
...
<div id="mainContent" class="mainContent u_floatL" style="width:740px;">
	...
	<table class="table">
		...
		<tr class="<%=(isOdd)?"odd":"even"%>">
			...
			<td style="width:50px"><a href="/partner/load-airline-deals?selectedSideNav=packages&dealId=<%=retDeal.getId()%>">Edit</a></td>
			<% if(retDeal.isDisabled()) { %>
			<td style="width:50px"><a href="/partner/load-airline-deals?selectedSideNav=packages&dealId=<%=retDeal.getId()%>&enable=true">Enable Sale</a></td>
			<% } else { %>
			<td style="width:50px"><a href="/partner/load-airline-deals?selectedSideNav=packages&dealId=<%=retDeal.getId()%>&enable=false">Disable Sale</a></td>
			<% } %>
		</tr>
		<% } %>
	</table>
	<% } %>
	<div class="spacer"></div>
	<h5 class="widget-name">Add Ancillaries</h5>
	<form class="form-horizontal" name="packageForm" action="/partner/add-airline-deals" method="post">
						...
						<div class="row-fluid">
							...
							<div class="span2">
								<select name="pricingType" id="pricingType">
									<option value="<%=AllowedPricingPredefined.PRICE_FULL_AMOUNT.getDesc()[0]%>" <%=(pricingKey != null && AllowedPricingPredefined.PRICE_FULL_AMOUNT == pricingKey)?"selected":""%>><%=AllowedPricingPredefined.PRICE_FULL_AMOUNT.getDesc()[0]%></option>
									...
									<option value="<%=AllowedPricingPredefined.PRICE_PER_DAY.getDesc()[0]%>" <%=(pricingKey != null && AllowedPricingPredefined.PRICE_PER_DAY == pricingKey)?"selected":""%>><%=AllowedPricingPredefined.PRICE_PER_DAY.getDesc()[0]%></option>
								</select>
							</div>
						</div>
					</div>
